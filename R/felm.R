  ...
  # then the clustered covariance matrix
    if(!is.null(cluster)) {
      ...
      if(method == 'cgm') {
        ...
        if(psdef) {
          ev <- eigen(cvcv)
          if(any(is.complex(ev$values)) | any(is.complex(ev$vectors)) | any(Re(ev$values) < 0)) {
            ...
            if(any(is.complex(ev$vectors))) {
              warning('Imaginary component of complex eigenvalues set to zero in multiway clustered variance matrix. See felm(...,psdef=FALSE)')
            }
            if(any(Re(ev$values) < 0)) {
              warning('Negative eigenvalues set to zero in multiway clustered variance matrix. See felm(...,psdef=FALSE)')
            }
            cvcv <- Re(ev$vectors) %*% diag(pmax(Re(ev$values),0)) %*% t(Re(ev$vectors))
          }
          rm(ev)
        }
